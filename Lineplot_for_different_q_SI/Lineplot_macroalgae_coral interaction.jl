###################### Line plot Figure.C1(b) in SI #######################################
################### For Macroalgae coral interaction model#######################
#############################################################################

using Distributed #package used for multiprocessing in julia
nprocs() #number of working cores
...
@everywhere function gen(q,ss) #=species response correlation, takes values q=0.0,0.45, 0.9 and
                                  ss is the number of replicates=#
  T=150000 #total time
  dt=0.1 #step size
  N=convert(Int,T/dt) #Number of iteration steps
  sig_X=0.02 #noise intensity for mutualist 1
  sig_Y=0.02 ##noise intensity for mutualist 2
  k=0.8 #fixed temporal correlation coefficient
  ...
  for ll in 1:ss
    ...
    x0=0.0025 #initial abundance of mutualist 1
    y0=0.5606 #initial abundance of mutualist 2
    ...
    for i in 2:N
      x[i]=x[i-1].+(x[i-1].*(0.1.*y[i-1].-(m[i-1]./(1-y[i-1])).+0.77.*(1.0.-x[i-1].-y[i-1])).+0.005.*(1-x[i-1]-y[i-1])).*dt.+x[i-1].*sig_X.*noise[i-1,1]
      y[i]=y[i-1].+(y[i-1].*(0.55.*(1.0.-x[i-1].-y[i-1]).-0.24.-0.1.*x[i-1])).*dt.+y[i-1].*sig_Y.*noise[i-1,2]
      m[i]=m[i-1]-(0.4/T)*dt
    end
...
legendfontsize=11,linewidth=2.5,color=:red2,thickness_scaling=1.1,grid=false,widen=false,xlim=(0.2,0.36),
legend_column=-1,legend=:outertop,
xlab=L"Maximum \; grazing\; rate\; (\beta)",tickfontsize=13,guidefontsize=17,ylab=L"Coral\; cover\; (C)")
...
legendfontsize=11,linewidth=2.5,color=:darkorange,thickness_scaling=1.1,grid=false,widen=false,xlim=(0.2,0.36),
legend_column=-1,
xlab=L"Maximum \; grazing\; rate (\beta)",tickfontsize=13,guidefontsize=17,ylab=L"Coral\; cover\; (C)")
...
legendfontsize=11,linewidth=2.5,color=:darkgreen,thickness_scaling=1.1,grid=false,widen=false,xlim=(0.2,0.36),
legend_column=-1,
xlab=L"Maximum \; grazing\; rate (\beta)",tickfontsize=13,guidefontsize=17,ylab=L"Coral\; cover\; (C)")

vline!([0.216],linewidth=1.5,linestyle=:dash,color=:darkgrey,label="",dpi=750)
