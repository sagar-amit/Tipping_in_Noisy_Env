###################### Line plot in Fig.3 (b) of the main text#######################################
################### MODIFIED RM MODEL (PREY-PREDATOR SYSTEM) ###############################################################
#####################################################################################################

using Distributed #package used for multiprocessing in julia
nprocs()   #number of working processors
addprocs(15)# add processors
w=workers()#set the worker processors
...
@everywhere function gen(q,ss) #species response correlation, takes values q=0.0,0.45, 0.9 and ss is the number of replicates
    ...
    k=0.6 # temporal correlation coefficient
    ...
    for ll in 1:ss
        ...
        x0=0.00295 #initial prey density at T=0
        y0=0.508  #initial predator density at T=0
...
#by using [1:100:end,:], only every 100th data point is retained for computation efficiency (reducing memory usage and computation time), 
# but one can use M=mat[1:1:end,:], R=m[1:1:end,:] (i.e., whole data) this will not change the result but may increase memory usage and computation time.


mn=mean(M,dims=2)#calculate mean across the replicates at each time point
...
####plot the mean trajectory with 5th and 95th percentile region for q=0.0
...
grid=false,xlab=L"Predator \; mortality\; rate\; (m)",widen=false,color=:red2,
tickfontsize=13,guidefontsize=17,ylab=L"Prey\; abundance \;(x)",xlim=(0.02,0.14))



#repeat the process above for different q values
q=0.45
...
####plot the mean trajectory with 5th and 95th percentile region for q=0.45
@df data plot!(:para,:mn,ribbon=(:mn-:p5s,-:mn+:p95s),fillalpha=0.1,framestyle=:box,label="q=0.45",foreground_color_legend=nothing,
legendfontsize=11,linewidth=1.75,thickness_scaling=1.1,legend=:outertop,legend_column=-1,
grid=false,xlab=L"Predator \; mortality\; rate\; (m)",widen=false,color=:darkorange,
tickfontsize=13,guidefontsize=17,ylab=L"Prey\; abundance \;(x)",xlim=(0.02,0.14))
...
####plot the mean trajectory with 5th and 95th percentile region for q=0.9
@df data plot!(:para,:mn,ribbon=(:mn-:p5s,-:mn+:p95s),fillalpha=0.1,framestyle=:box,label="q=0.9",foreground_color_legend=nothing,
legendfontsize=11,linewidth=1.75,thickness_scaling=1.1,legend=:outertop,legend_column=-1,
grid=false,xlab=L"Predator \; mortality\; rate\; (m)",widen=false,color=:darkgreen,
tickfontsize=13,guidefontsize=17,ylab=L"Prey\; abundance \;(x)",xlim=(0.02,0.14))

## plot actual tipping point in deterministic system as a vertical line
vline!([0.128],linewidth=1.5,linestyle=:dash,color=:darkgrey,label="",dpi=750)




# function to generate trajectories if the control parameter varies in the backward direction
@everywhere function gen_rev(q,ss)
    ...
    for ll in 1:ss
        ...
        k1=1.2  #carrying capacity for the predator populatiion
        d=6     #conversion of biomass from predator to prey (d=c*a)
        m0=0.15 #initial value of the control parameter (predator mortality rate)
     
        x0=0.663 #initial prey density at T=0
        y0=0.945  #initial predator density at T=0
...
vline!([0.03],linewidth=1.5,linestyle=:dash,color=:darkgrey,label="",dpi=750)
